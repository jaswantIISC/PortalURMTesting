...
public class NodeUpdateTestCases {
	...
	public OneM2MResponse updateLabels(List<String> labels, String to, String from, SecurityParams securityParams) {
		...
		try {
			...
			oneM2MRequest = ComposeRequest.oneM2MRequestUpdate(to, from, node);
	...
	public OneM2MResponse updateAccessControlPolicyIds(List<String> acpi, String to, String from, SecurityParams securityParams) {
		...
		try {
			Node node = new Node();
			node.setAccessControlPolicyIDs(acpi);
			oneM2MRequest = ComposeRequest.oneM2MRequestUpdate(to, from, node);
			oneM2MResponse = originatorActions.execute(oneM2MRequest, TestingParams.getCSEPoA(), securityParams);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public OneM2MResponse updateHostedCSELinks(String hostedCSELink, String to, String from, SecurityParams securityParams) {
		...
		try {
			Node node = new Node();
			node.setHostedCSELink(hostedCSELink);
			oneM2MRequest = ComposeRequest.oneM2MRequestUpdate(to, from, node);
			oneM2MResponse = originatorActions.execute(oneM2MRequest, TestingParams.getCSEPoA(), securityParams);
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public OneM2MResponse updateNodeId(String nodeId, String to, String from, SecurityParams securityParams) {
