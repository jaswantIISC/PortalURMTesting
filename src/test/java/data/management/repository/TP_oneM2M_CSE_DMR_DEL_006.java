...
@Tag("TP_oneM2M_CSE_DMR_DEL_006")
public class TP_oneM2M_CSE_DMR_DEL_006 {
	...
	@Test
	@Tag("TP_oneM2M_CSE_DMR_DEL_006_CNT_CIN")
	void TP_oneM2M_CSE_DMR_DEL_006_CNT_CIN() {
		...
		BigInteger currentNrOfInstnaceOld = cont.getCurrentNrOfInstances();
		BigInteger currentByteSizeOld = cont.getCurrentByteSize();
		...
		assertEquals(M2MStatus.OK.getOnem2mStatusCode(), container1Response.getResponseStatusCode());
		cont = (Container) container1Response.getPrimitiveContent().getAny();
		BigInteger currentNrOfInstnaceNew = cont.getCurrentNrOfInstances();
		BigInteger currentByteSizeNew = cont.getCurrentByteSize();
		
		assertTrue(currentNrOfInstnaceNew.intValue() < currentNrOfInstnaceOld.intValue());
		assertTrue(currentByteSizeNew.intValue() < currentByteSizeOld.intValue());
	...
	@Test
	@Tag("TP_oneM2M_CSE_DMR_DEL_006_TS_TSI")
	void TP_oneM2M_CSE_DMR_DEL_006_TS_TSI() {
		...
		BigInteger currentNrOfInstnaceOld = ts.getCurrentNrOfInstances();
		BigInteger currentByteSizeOld = ts.getCurrentByteSize();
		...
		assertEquals(M2MStatus.OK.getOnem2mStatusCode(), tsResponse.getResponseStatusCode());
		ts = (TimeSeries) tsResponse.getPrimitiveContent().getAny();
		BigInteger currentNrOfInstnaceNew = ts.getCurrentNrOfInstances();
		BigInteger currentByteSizeNew = ts.getCurrentByteSize();
