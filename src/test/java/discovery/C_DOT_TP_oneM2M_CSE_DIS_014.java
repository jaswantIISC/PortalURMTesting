...
import cdot.onem2m.common.enums.AccessControlOperations;
...
@Tag("C_DOT_TP_oneM2M_CSE_DIS_014")
public class C_DOT_TP_oneM2M_CSE_DIS_014 {
	...
	// Test case for DesiredIdentifierResultType = 2
	@Test
	@Tag("C_DOT_TP_oneM2M_CSE_DIS_014_DIS")
	void C_DOT_TP_oneM2M_CSE_DIS_014_DIS() {
		...
		filCri.setResourceType(Arrays.asList(M2MSimpleResources.CONTENTINSTANCE.getM2MResourceTypes()));

				
				
		OneM2MResponse disRes = genRet.retrieveForDiscoveryAndDRI(BigInteger.valueOf(2), filCri, container1StructuredResourceId, TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		assertEquals(M2MStatus.OK.getOnem2mStatusCode(), disRes.getResponseStatusCode());
		assertTrue(disRes.getPrimitiveContent().getAny() instanceof ArrayList<?>);
		List<String> listURI = (List<String>) disRes.getPrimitiveContent().getAny();
		assertTrue(!listURI.isEmpty());
