...
@Tag("TP_oneM2M_CSE_DIS_005")
public class TP_oneM2M_CSE_DIS_005 {
	...
	@Test
	@Tag("TP_oneM2M_CSE_DIS_005_DIS")
	void TP_oneM2M_CSE_DIS_005_DIS() {
		...
		acr.setAccessControlOperations(BigInteger.valueOf(AccessControlOperations.CR_RE_DE_NO.getValue()));
		setOfACRs.setAccessControlRule(Arrays.asList(acr));
		OneM2MResponse acpResponse = acpCreate.createWithCustomPrivilegeSelfPrivilege(setOfACRs, setOfACRs, TestingParams.CSE_BASE_RESOURCE_NAME, TestingParams.ADMIN_AE_ID, TestingParams.getSecurityParams(TestingParams.ADMIN_AE_ID));
		...
		OneM2MResponse conRes = conCreate.containerWithExternalACPIDs(TestingParams.CSE_BASE_RESOURCE_NAME, TestingParams.ADMIN_AE_ID, TestingParams.getSecurityParams(TestingParams.ADMIN_AE_ID), Arrays.asList(acp.getResourceID()));
		...
		OneM2MResponse disRes = genRet.retrieveForDiscovery(filCri, container1ResourceId, TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		assertEquals(M2MStatus.OK.getOnem2mStatusCode(), disRes.getResponseStatusCode());
		List<String> uriList = (List<String>) disRes.getPrimitiveContent().getAny();
		assertTrue(uriList.isEmpty());
		
		
		GeneralDeleteTestCases genDel = new GeneralDeleteTestCases();
		OneM2MResponse delRes = genDel.deleteResource(container1ResourceId, TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		assertEquals(delRes.getResponseStatusCode(), M2MStatus.DELETED.getOnem2mStatusCode());
		// INITIAL STATE ENDS
