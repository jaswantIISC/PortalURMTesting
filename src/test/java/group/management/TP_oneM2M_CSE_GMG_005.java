...
@Tag("TP_oneM2M_CSE_GMG_005")
public class TP_oneM2M_CSE_GMG_005 {
	...
	@Test
	@Tag("TP_oneM2M_CSE_GMG_005_CRE")
	void TP_oneM2M_CSE_GMG_005_CRE() {
		...
		String ae1StructuredResourceID = aeStructuredResourceID + "/" + ae.getResourceName();
		...
		String ae2StructuredResourceID = aeStructuredResourceID + "/" + ae.getResourceName();

		GroupCreate grpCreate = new GroupCreate();
		Group grp = new Group();
		grp.setMemberIDs(Arrays.asList(ae1ResourceID, ae2ResourceID));
		...
		OneM2MResponse fanRes = contCreate.createWithAnyAttribute(group.getResourceID() + "/fopt", TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		...
		listCIs.add(ae1ResourceID);
		listCIs.add(ae2ResourceID);
	...
	@Test
	@Tag("TP_oneM2M_CSE_GMG_005_RET")
	void TP_oneM2M_CSE_GMG_005_RET() {
		aeStructuredResourceID = TestingParams.CSE_BASE_RESOURCE_NAME;
		...
		listCIs.add(ae2ResourceID);
		for (ResponsePrimitive indiRes : listRes) {
			assertTrue(indiRes.getPrimitiveContent().getAny() instanceof AE);
			ae = (AE) indiRes.getPrimitiveContent().getAny();
			assertTrue(listCIs.contains(ae.getResourceID()));
		}
	...
	@Test
	@Tag("TP_oneM2M_CSE_GMG_005_UPD")
	void TP_oneM2M_CSE_GMG_005_UPD() {
		ContainerCreateTestCases contCreate = new ContainerCreateTestCases();
		aeStructuredResourceID = TestingParams.CSE_BASE_RESOURCE_NAME;
		...
		OneM2MResponse fanRes = aeUpd.updateLabel(listString, group.getResourceID() + "/fopt", TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		assertEquals(M2MStatus.OK.getOnem2mStatusCode(), fanRes.getResponseStatusCode());
		assertTrue(fanRes.getPrimitiveContent().getAny() instanceof AggregatedResponse);
		List<ResponsePrimitive> listRes = ((AggregatedResponse) fanRes.getPrimitiveContent().getAny()).getResponsePrimitive();
		...
		for (ResponsePrimitive indiRes : listRes) {
			...
			ae = (AE) indiRes.getPrimitiveContent().getAny();
			assertTrue(listCIs.contains(ae.getResourceID()));
			assertEquals(ae.getLabels(), listString);
		}
	...
	@Test
	@Tag("TP_oneM2M_CSE_GMG_005_DEL")
	void TP_oneM2M_CSE_GMG_005_DEL() {
		ContainerCreateTestCases contCreate = new ContainerCreateTestCases();
		aeStructuredResourceID = TestingParams.CSE_BASE_RESOURCE_NAME;
