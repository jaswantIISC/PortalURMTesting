...
@Tag("TP_oneM2M_CSE_GMG_CRE_006")
public class TP_oneM2M_CSE_GMG_CRE_006 {
	...
	@Test
	@Tag("TP_oneM2M_CSE_GMG_CRE_006_CONT")
	void TP_oneM2M_CSE_GMG_CRE_006_CONT() {
		...
		container1Response = ciCreate.createWithAnyAttribute(container1ResourceId, TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		assertEquals(M2MStatus.CREATED.getOnem2mStatusCode(), container1Response.getResponseStatusCode());
		// CONTAINER CREATE 1 ENDS
		assertTrue(container1Response.getPrimitiveContent().getAny() instanceof ContentInstance);
		ContentInstance ci = (ContentInstance) container1Response.getPrimitiveContent().getAny();
		String ciResourceId = ci.getResourceID();
		String ciStructuredResourceId = container1StructuredResourceId + "/" + cont.getResourceName();

		GroupCreate grpCreate = new GroupCreate();
		Group grp = new Group();
		grp.setMemberIDs(Arrays.asList(container1ResourceId, ciResourceId));
		...
		assertEquals(M2MStatus.CREATED.getOnem2mStatusCode(), grpRes.getResponseStatusCode()); // 5105 status code not available in M2MStatus ENUM
		assertTrue(grpRes.getPrimitiveContent().getAny() instanceof Group);
		Group group = (Group) grpRes.getPrimitiveContent().getAny();
		assertTrue(group.getMemberTypeValidated() == true);
		assertTrue(group.getMemberIDs().size() == 1);
		assertTrue(group.getMemberIDs().contains(container1ResourceId));
