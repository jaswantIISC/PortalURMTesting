...
@Tag("TP_oneM2M_CSE_SUB_CRE_008")
public class TP_oneM2M_CSE_SUB_CRE_008 {
	...
	@Test
	@Tag("TP_oneM2M_CSE_SUB_CRE_008_GRP")
	void TP_oneM2M_CSE_SUB_CRE_008_GRP() {
		...
		String containerResourceId2 = cont.getResourceID();
		String containerStructuredResourceId2 = aeStructuredResourceID + "/" + cont.getResourceName();
		
		
		GroupCreate grpCreate = new GroupCreate();
		OneM2MResponse grpResponse = grpCreate.createWithMemberIds(Arrays.asList(container1StructuredResourceId, containerStructuredResourceId2), TestingParams.GENERAL_ADN_AE_ID, TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		...
		sub.setGroupID(grp.getResourceID());
		sub.setNotificationURI(Arrays.asList("http://IP:PORT"));
		sub.setNotificationForwardingURI("http://IP:PORT");
		OneM2MResponse subResponse = subCreate.createWithSubscription(sub, grp.getResourceID() + "/fopt", TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		assertEquals(M2MStatus.OK.getOnem2mStatusCode(), subResponse.getResponseStatusCode());
		assertTrue(subResponse.getPrimitiveContent().getAny() instanceof AggregatedResponse);
		AggregatedResponse aggRes = (AggregatedResponse) subResponse.getPrimitiveContent().getAny();
		assertEquals(aggRes.getResponsePrimitive().size(), grp.getCurrentNrOfMembers().intValue());
		for (ResponsePrimitive response : aggRes.getResponsePrimitive()) {
			assertEquals(M2MStatus.CREATED.getOnem2mStatusCode(), response.getResponseStatusCode());
		}
		// INITIAL STATE ENDS
