...
@Tag("TP_oneM2M_CSE_SUB_UPD_005_CUSTOM")
public class TP_oneM2M_CSE_SUB_UPD_005_CUSTOM {
	...
	@Test
	@Tag("TP_oneM2M_CSE_SUB_UPD_005_CUSTOM_EC")
	void TP_oneM2M_CSE_SUB_UPD_005_CUSTOM_EC() {
		...
		while (flag) {
			System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CI CREATE COUNTER : " + requestCounter++ + "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
			...
			if (subRetrieve.getResponseStatusCode().intValue() == M2MStatus.NOT_FOUND.getOnem2mStatusCode().intValue()) {
				flag = false;
			}
		}
		...
		OneM2MResponse subRetrieve = genRet.retrieveResource(sub.getResourceID(), TestingParams.GENERAL_ADN_AE_ID, TestingParams.getSecurityParams(TestingParams.GENERAL_ADN_AE_ID));
		assertEquals(M2MStatus.NOT_FOUND.getOnem2mStatusCode(), subRetrieve.getResponseStatusCode());
